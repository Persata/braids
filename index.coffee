...
class BraidBase
			...
			for field in @.fields
				...
				if allErrors or !@.errorMessages[field]?
					...
					if @.fileValidators[field]?
						result = new @.fileValidators[field]().validate(field, @, allErrors)
	...
	class @.FileValidator
		...
		validate: (field, model, allErrors) =>
			...
			if @.required is true
				# Check Name and Size
				if fileObject.size is 0 or fileObject.name is '' or fileObject.originalFilename is ''
			...
			if @.maxFileSize
				...
				console.log filesize(@.maxFileSize).to('MB')
			# Return At The End
			return overallResult
